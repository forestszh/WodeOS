...
# PROGRAM DESCRIPTION
#
#	This script lists the files in sysroot that are not in a package list,
#	or are in more than one package list.  The package lists are in
#	sysroot/usr/share/ttylinux.
#
# CHANGE LOG
#
...
for f in $(find sysroot/ -type f); do
	...
	case ${hit} in
		...
		1)	;;
		*)	echo "=> Multiply-claimed file \"sysroot/${f}\""
			for p in ${hitList}; do
				echo "-> $(basename ${p})"
			done
			;;
